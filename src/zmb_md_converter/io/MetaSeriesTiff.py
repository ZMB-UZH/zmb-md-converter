"""
This file is adapted from https://github.com/fmi-faim/faim-ipa.

TODO: Pull request to faim-ipa, to include Time metadata.
"""
...
def load_metaseries_tiff_metadata(path: Path) -> dict:
    ...
    * Timepoint (if existent)
    * acquisition-time-local (if existent)
    ...
    with tifffile.TiffFile(path) as tiff:
        ...
        selected_keys = [
            ...
            "Timepoint",  # optional
            "acquisition-time-local",  # optional
        ]
...
def load_metaseries_tiff(path: Path) -> tuple[ArrayLike, dict]:
    """Load a metaseries tiff image and metadata."""
    with tifffile.TiffFile(path) as tiff:
        data = tiff.asarray()
